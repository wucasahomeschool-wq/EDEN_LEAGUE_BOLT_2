...
function extractJson<T>(content: string): T | null {
  ...
  if (arrStart !== -1) {
    ...
    if (end > arrStart) {
      const r = tryParse(content.slice(arrStart, end + 1));
      if (r) return r;
    }
  }
  if (objStart !== -1) {
    const end = content.lastIndexOf("}");
    if (end > objStart) {
      const r = tryParse(content.slice(objStart, end + 1));
      if (r) return r;
    }
  }
  return null;
  ...
  .handler(async ({ data }) => {
    const reqLines = data.specialRequests.length
      ? data.specialRequests
          .map(
            (r) =>
              `  - ${r.home} vs ${r.away}${r.week ? ` (must be in Week ${r.week})` : " (any week)"}`,
          )
          .join("\n")
      : "  - (none)";
    ...
    const goal =
      data.phase === "finalfour"
        ...
        : data.phase === "preseason"
          ? `This is the PRE-SEASON (Weeks ${data.weeks.join(", ")}). These are warm-up matches that don't count toward standings. Build a balanced, entertaining set of matchups so every team gets a good shake-out. Avoid repeating the same pairing.`
...
export const fixScheduleWeek = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const parsed = extractJson<
      { home: string; away: string }[] | { fixtures?: { home: string; away: string }[] }
    >(content);
    const list = Array.isArray(parsed) ? parsed : parsed?.fixtures;
