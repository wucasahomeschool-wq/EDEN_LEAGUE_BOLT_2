...
export function DraftSuite() {
  ...
  // The draft is now handled by the separate offseason app. This suite is
  // kept for viewing draft picks, but the draft itself runs in the offseason app.
  // Draft picks and prospects come back via the Offseason Import suite.
  if (!draft?.started) {
    return (
      <div className="rounded-xl border bg-card p-8 text-center">
        <h2 className="text-lg font-extrabold uppercase tracking-wide">
          Eden League Draft — Moved to Offseason App
        </h2>
        <p className="mx-auto mt-2 max-w-md text-sm text-muted-foreground">
          The draft is now run in the separate Eden League Offseason app. After the season ends,
          use the <strong className="text-foreground">Offseason Import</strong> suite to import the
          offseason data (including draft results, prospects, and picks) and advance to the next season.
        </p>
      </div>
    );
  }
...
function ProspectCreator({
  ...
  async function generate() {
    ...
    setLoading(true);
    try {
      const { attributes } = await genRatings({
        data: {
          name: name.trim(),
            ...
            <div className="mt-3">
              ...
              <div className="flex flex-wrap gap-2">
                <Button
                  ...
                  onClick={() =>
                    setDescription(
                      "This player is extremely fast and strong, has great shooting power, but possesses very poor defending and tackling.",
                    )
                  }
                >
                  ⚡ Speedster/Target Man
                </Button>
                <Button
                  id="preset-playmaker"
                  type="button"
                  ...
                  onClick={() =>
                    setDescription(
                      "A brilliant, intelligent midfielder with world-class passing and high vision, but is physically weak and has slow pace.",
                    )
                  }
                >
                  🧠 Midfield Maestro
                ...
                <Button
                  ...
                  onClick={() =>
                    setDescription(
                      "A absolute wall of a defender: extremely strong, aggressive, and has superb tackling, but has low speed and poor finishing.",
                    )
                  }
                >
                  🧱 Rock-Solid Wall
